...
import edu.ksu.cis.bandera.jext.ExternalChooseExpr;
...
/**
 * The ExternalIntRangeChooseExpr represents a specialized choose expression
 * that takes a range of integers to choose from.  Since it extends
 * ExternalChooseExpr, it implements the getChoices() method but will actually
 ...
 */
public class ExternalIntRangeChooseExpr extends ExternalChooseExpr {

    protected int start;
    protected int end;

    public ExternalIntRangeChooseExpr(int start, int end) {
	super(new ArrayList());
	this.start = start;
	this.end = end;
    }
	...
	return "ExternalChoose(" + getChoices() + ")";
    }  
}
