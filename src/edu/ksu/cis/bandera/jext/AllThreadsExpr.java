...
import java.util.*;
public class AllThreadsExpr implements UnopExpr {
	protected ValueBox opBox;
/**
 * 
 */
public AllThreadsExpr(Value v) {
	this(Grimp.v().newArgBox(v));
}
/**
 * ComplementExpr constructor comment.
 */
public AllThreadsExpr(ValueBox valueBox) {
	opBox = valueBox;
}
/**
...
public void apply(ca.mcgill.sable.util.Switch sw) {
    System.out.println("sw.getClass().getName() = " + sw.getClass().getName());
	((BanderaSpecExprSwitch) sw).caseAllThreadsExpr(this);
}
/**
...
public String toBriefString() {
	return "A t. " + ((ToBriefString) opBox.getValue()).toBriefString();
}
/**
 * 
